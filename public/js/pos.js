...
(function() {
    ...
    function searchCustomer() {
        ...
        if (!keyword) {
            ...
            showNotification('⚠️ Vui lòng nhập SĐT hoặc tên khách hàng', 'warning');
            return;
        }
    ...
    function applyDiscount(type, value) {
        ...
        if (type === 'fixed' && value < 0) {
            showNotification('Số tiền giảm không hợp lệ', 'error');
            return;
        }

        // If discount > 20%, require manager password
        if ((type === 'percent' && value > 20) || (type === 'fixed' && value > cart.subtotal * 0.2)) {
            const password = prompt('Giảm giá lớn yêu cầu mật khẩu quản lý:');
            if (!password) {
                return;
            }
        ...
        fetch(CONFIG.apiEndpoints.discount, {
            ...
            body: JSON.stringify({
                ...
                value: value,
                reason: 'Manual discount by cashier'
            })
        })
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                updateCartFromServer();
                showNotification('Đã áp dụng giảm giá', 'success');
            } else {
    ...
    function showDiscountModal() {
        ...
        // Example with native prompt:
        const type = prompt('Loại giảm giá (percent/fixed):');
        if (!type || (type !== 'percent' && type !== 'fixed')) {
            return;
        }

        const value = parseFloat(prompt('Giá trị giảm:'));
        if (isNaN(value)) {
            return;
        }
    ...
    function confirmPayment() {
        ...
        if (selectedPaymentMethod === 'cash') {
            ...
            paymentData.change = paymentData.cash_received - cart.total;
        }
    ...
    function proceedToPayment() {
        ...
        paymentModal.show();
        showNotification('💳 Chọn phương thức thanh toán', 'info');
    }
